...
#Region " Type declarations "
    ...
    Public Enum TagNames As Integer
        ...
        JPEGQuality = &H5010 '(Private tag used by the Adobe Photoshop format. Not for public use.)
        GridSize = &H5011
        ...
        ExifISOSpeed = &H8827        'ISO speed and ISO latitude of the camera or input device as specified in ISO 12232.
        ExifOECF = &H8828
        ExifVer = &H9000
        ExifDTOrig = &H9003
        ...
        ExifCompBPP = &H9102
        ExifShutterSpeed = &H9201    'Shutter speed. The unit is the Additive System of Photographic Exposure (APEX) value.
        ExifAperture = &H9202
        ...
        ExifFlash = &H9209
        ExifFocalLength = &H920A     'Actual focal length, in millimeters, of the lens. Conversion is not made to the focal length of a 35 millimeter film camera.
        ExifMakerNote = &H927C       'Note tag. A tag used by manufacturers of EXIF writers to record information. The contents are up to the manufacturer.
        ExifUserComment = &H9286     'Comment tag. A tag used by EXIF users to write keywords or comments about the image besides those in PropertyTagImageDescription and without the character-code limitations of the PropertyTagImageDescription tag.
        ExifDTSubsec = &H9290        'Null-terminated character string that specifies a fraction of a second for the PropertyTagDateTime tag.
        ExifDTOrigSS = &H9291
        ExifDTDigSS = &H9292
        ExifFPXVer = &HA000
...
#End Region
    ...
    ''' Initializes new instance of this class.
    ''' </summary>
    ''' <param name="PathedFileName">Name and path of file to be loaded</param>
    ''' <remarks></remarks>
    ''' <history>
    ''' [altair] 13.06.2004 Created
    ''' </history>
    Public Sub New(ByVal PathedFileName As String)
        Me._Image = DirectCast(System.Drawing.Bitmap.FromFile(PathedFileName), System.Drawing.Bitmap)
    End Sub
    ...
    ''' <summary>
    ''' Returns some data in formatted string form
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ''' <history>
    ...
    Public Overrides Function ToString() As String
        ...
        SB.Append("Image:")
        SB.Append("\n\tDimensions:        " + Me.Width.ToString() + " x " + Me.Height.ToString() + " px")
        SB.Append("\n\tResolution:        " + Me.ResolutionX.ToString() + " x " + Me.ResolutionY.ToString() + " dpi")
        ...
        SB.Append("\n\tISO sensitivity:   " + Me.ISO.ToString())
        SB.Append("\n\tSubject distance:  " + Me.SubjectDistance.ToString("N2") + " m")
        SB.Append("\n\tFocal length:      " + Me.FocalLength.ToString())
...
#Region " Nicely formatted well-known properties "
    ...
    ''' <summary>
    ''' Decimal metres of altitude
    ''' </summary>
    ''' <returns></returns>
    Public ReadOnly Property Altitude() As Double
        Get
            If IsPropertyDefined(TagNames.GpsAltitude) Then
                Dim altRef As String = "00"
                Try
                    Dim Ref As Byte() = GetProperty(TagNames.GpsAltitudeRef)
                    altRef = BitConverter.ToString(Ref, 0, 1)
                Catch ex As Exception
                End Try
                Dim alt As Rational = GetPropertyRational(TagNames.GpsAltitude)
                Dim hgt As [Double] = alt.ToDouble()
                If Not altRef.ToString.Equals("00") Then ' (Above Sea Level = 0, Below = 1)
                    hgt = -hgt
                End If
                Return hgt
            Else
    ...
    ''' </summary>
    ''' <returns></returns>
    Public ReadOnly Property str_Latitude() As String
        Get
            If IsPropertyDefined(TagNames.GpsLatitude) Then
                Dim oPropertyItem_01 As System.Drawing.Imaging.PropertyItem
                oPropertyItem_01 = Me._Image.GetPropertyItem(ExifWorks.TagNames.GpsLatitude)
                Dim Components As Byte() = oPropertyItem_01.Value

                Dim n_Degrees As Double = CType(BitConverter.ToInt32(Components, 0), Double) / BitConverter.ToInt32(Components, 4)
                Dim n_Minutes As Double = CType(BitConverter.ToInt32(Components, 8), Double) / BitConverter.ToInt32(Components, 12)
                Dim n_Seconds As Double = CType(BitConverter.ToInt32(Components, 16), Double) / BitConverter.ToInt32(Components, 20)

                Return n_Degrees.ToString() + "° " + n_Minutes.ToString() + "' " + n_Seconds.ToString() + """ " + GetPropertyString(ExifWorks.TagNames.GpsLatitudeRef).ToString()
            Else
                Return ""
            End If
        End Get
    ...
    ''' </summary>
    ''' <returns></returns>
    Public ReadOnly Property str_Longitude() As String
        Get
            If IsPropertyDefined(TagNames.GpsLongitude) Then
                Dim oPropertyItem_01 As System.Drawing.Imaging.PropertyItem
                oPropertyItem_01 = Me._Image.GetPropertyItem(ExifWorks.TagNames.GpsLongitude)
                Dim Components As Byte() = oPropertyItem_01.Value
                ...
                Return n_Degrees.ToString() + "° " + n_Minutes.ToString() + "' " + n_Seconds.ToString() + """ " + GetPropertyString(ExifWorks.TagNames.GpsLongitudeRef).ToString()
            Else
                Return ""
            End If
        End Get
    ...
    ''' </summary>
    ''' <returns></returns>
    Public ReadOnly Property n_Latitude() As Double
        Get
            If IsPropertyDefined(TagNames.GpsLatitude) Then
                Dim oPropertyItem_01 As System.Drawing.Imaging.PropertyItem
                oPropertyItem_01 = Me._Image.GetPropertyItem(ExifWorks.TagNames.GpsLatitude)
                ...
                Dim n_DecimalDegrees As Double = n_Degrees + n_Minutes / 60 + n_Seconds / 3600
                If (GetPropertyString(ExifWorks.TagNames.GpsLongitudeRef) = "S") Then
                    n_DecimalDegrees = -n_DecimalDegrees
                End If

                Return n_DecimalDegrees
            Else
                Return 0
            End If
        End Get
    ...
    ''' </summary>
    ''' <returns></returns>
    Public ReadOnly Property n_Longitude() As Double
        Get
            If IsPropertyDefined(TagNames.GpsLongitude) Then
                Dim oPropertyItem_01 As System.Drawing.Imaging.PropertyItem
                oPropertyItem_01 = Me._Image.GetPropertyItem(ExifWorks.TagNames.GpsLongitude)
                ...
                Dim n_DecimalDegrees As Double = n_Degrees + n_Minutes / 60 + n_Seconds / 3600
                If (GetPropertyString(ExifWorks.TagNames.GpsLongitudeRef) = "W") Then
                    n_DecimalDegrees = -n_DecimalDegrees
                End If
